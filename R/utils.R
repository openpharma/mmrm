...
#' Validate an mmrm Fit
#' @param object (`mmrm`)\cr object.
#' @keywords internal
#' @details
#' During mmrm fitting, sometimes the fit itself converged but
#' the fit is not valid. This function asserts the fit is valid
#' through the covariance and coefficient covariance.
h_valid_fit <- function(object) {
  h_qr(
    object$cov,
    "Variance parameter covariance is not positive definite! The model might have been over-parameterized!"
  )
  h_qr(
    object$beta_vcov,
    "Coefficient covariance is not positive definite! The model might have been over-parameterized!"
  )
}

#' Validation with QR Decomposition
#' @param x (`list`)\cr or (`matrix`)\cr input to be checked.
#' @param msg (`string`)\cr message to raise on error.
#' @keywords internal
h_qr <- function(x, msg) {
  assert_string(msg)
  if (is.list(x)) {
    lapply(x, h_qr, msg = msg)
    return()
  }
  if (!(test_matrix(x, any.missing = FALSE) && all(is.finite(x)))) {
    stop(msg)
  }
  qr_res <- qr(x)
  if (qr_res$rank < ncol(x)) {
    stop(msg)
  }
}
