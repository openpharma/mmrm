...
For determining the denominator degrees of freedom (DDF) required for the testing of fixed effects, one option is to use the 'between-within' method.

Using this method, the DDF are determined by the grouping level at which the term is estimated. In the case of an MMRM model with only fixed effect terms, this method simplifies to including 3 potential 'levels' of parameters (@galecki2013linear):
...
With an MMRM with only fixed terms we have a single grouping level (e.g. subject), $G=1$. Therefore:
...
* for the RACE effect we estimate 2 parameters
* for the SEX effect we estimate 1 parameter
* for the ARMCD effect we estimate 1 parameter
* for the AVISIT effect we estimate 3 parameters
* for the interaction of ARMCD and AVISIT we estimate 3 parameters

Therefore:

* $N_0=1$ since we have an intercept term
* $N_1=2+1+1=4$
* $N_2=3+3=6$
