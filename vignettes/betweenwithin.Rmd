...
* Level 0: The intercept term, assuming the model has been fitted with one. 
  - We use $DDF_2$ degrees of freedom.
...
In the model above, `RACE`, `SEX` and `ARMCD` are between-subjects effects; they do not vary within subject across the repeated observations. 
On the other hand, `AVISIT` is a within-subject effect; it represents study visit, so naturally its value changes over repeated observations for each subject.
