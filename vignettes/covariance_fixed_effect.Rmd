...
To estimate the covariance matrix of fixed effects, there are many ways.
...
For weighted mmrm, we can follow the [details of weighted leaset square estimator](https://openpharma.github.io/mmrm/main/articles/algorithm.html#weighted-least-squares-estimator).
...
\[
  cov(\hat\beta) = \frac{n-1}{n}(X^\top W X)^{-1}(\sum_{i}{X_i^\top L_{i} (I_{i} - H_{ii})^{-1} L_{i}^\top \hat\epsilon_i\hat\epsilon_i^\top L_{i} (I_{i} - H_{ii})^{-1} L_{i}^\top X_i})(X^\top W X)^{-1}
\]
