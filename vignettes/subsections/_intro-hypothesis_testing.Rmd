...
the effect in mmrm models. For example, in order to see if the used variables
are related to the response:

```{r car_type2}
library(car)
anova_result2 <- Anova(fit, type = "II")
anova_result2
```

Note that the degrees of freedom choice is inherited here from the initial `mmrm`
fit.
In addition, if you see results different from SAS, it can be because the reference level is
different.
We can also use type III hypothesis testing:

```{r car_type3}
anova_result3 <- Anova(fit, type = "III")
anova_result3
```
