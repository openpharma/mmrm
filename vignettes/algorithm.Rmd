...
### Linear model with group covariance matrix

In some cases, the covariance matrix is different in different groups, while the covariance structures remain the same across groups.
Following the notations in the previous section, for subject $i$ in group $j$, we have

\[
Y_{ij} \sim N(X_{ij}\beta, \Sigma_{ij})
\]
where $X_{ij}$ is the design matrix of subject $i$ in group $j$.
$\beta$ is shared across groups.

The covariance matrix $\Sigma_{ij} \in \mathbb{R}^{m_i \times m_i}$ is derived
by subsetting the overall covariance matrix $\Sigma_{j} \in \mathbb{R}^{m \times m}$
appropriately by

\[
\Sigma_{ij} = S_i^\top \Sigma_{j} S_i
\]

where $\Sigma_{j}$ is the covariance matrix of group $j$.
The resulting $\epsilon$ has similar property with covariance matrix $\Omega$ changed
...
where $\Omega \in \mathbb{R}^{N \times N}$ is block-diagonal containing the
subject specific $\Sigma_{ij}$ covariance matrices on the diagonal and 0 in the
remaining entries.
...
utilizing Newton-Raphson algorithms based on the gradient and Hessian.
