...
vector and covariance matrix. When weights are specified, we replace the covariance matrix $\Sigma_i$ with $Z_i^{-1/2} \Sigma_i Z_i^{-1/2}$, where $Z_i$ is the diagonal weight matrix.
...
of its covariance matrix. When the diagonal weight matrix $Z_i$ is specified, we replace $W_i$ with $W_i = (Z_i^{-1/2} \Sigma_i Z_i^{-1/2})^{-1} = Z_i^{1/2} \Sigma_i^{-1} Z_i^{1/2}$.
...
identity matrix as alternative right hand side. When weights $Z_i$ is specified, we replace $L_i$ with $Z_i^{-1/2} L_i$ in the above calculation.
...
When weights $Z_i$ is specified, we have $\frac{1}{2}\log\det(\Omega) = \sum_{i=1}^{n}\sum_{j=1}^{m_i}\log(z_{i, j} l_{i, jj})$, where $z_{i, j}$ are the diagonal entries of the weight $Z_i$.
