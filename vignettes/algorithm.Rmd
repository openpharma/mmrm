...
Assume that there are total number of $G$ groups, the length of $\theta$ is multiplied by $G$, and for each part, $\theta$ is parametrized in the same fasion.
Following unstructured covariance matrix, $\theta$ has $k = G * m(m+1)/2$ entries.
Similarly for other covariance matrix choices.
