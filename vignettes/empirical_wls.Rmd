...
Since they have nearlly the same symbols, to differentiate the different part, we use
subscript $wls$ to denote the implementation suggested by @bell2002bias and @pustejovsky2018small,
and use $vmmrm$ to denote the our implementation of covariance estimator in `mmrm`, we have
...
apprarently, if $d=0$, these two are identical because $W_i = L_i L_i^\top$.
...
\[
  L^{-1} g_{wls, i} = L^{-1} (I - X(X^\top W X)^{-1}X^\top W) S_i^\top A_{wls, i} W_i X_i (X^\top W X)^{-1} C
\]

\[
  g_{mmrm, i} = (I - L_i^\top X(X^\top W X)^{-1}X^\top L_i) S_i^\top A_{mmrm, i} L_i^\top X_i (X^\top W X)^{-1} C
\]
...
\[
  L^{-1} (I - X(X^\top W X)^{-1}X^\top W) S_i^\top A_{wls, i} W_i = (I - L^\top X(X^\top W X)^{-1}X^\top L) S_i^\top A_{mmrm, i} L_i^\top
\]
