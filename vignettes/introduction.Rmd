...
Grouped covariance structures is supported by `mmrm` package. 
Covariance matrix for each group are of identical structure(unstructured, compound symmetry, etc) but the values can be different.
...
We cam see that the covariance is different in different ARMCD group.
...
The lower level function which is called by [`mmrm()`](https://openpharma.github.io/mmrm/main/reference/mmrm.html) is [`h_mmrm_tmb()`](https://openpharma.github.io/mmrm/main/reference/h_mmrm_tmb.html). This 
...
h_mmrm_tmb(formula = FEV1 ~ RACE + ARMCD * AVISIT + us(AVISIT | USUBJID),
  data = fev_data,
  reml = TRUE,
...
For fine control of optimization routine, [`h_mmrm_tmb_control()`](https://openpharma.github.io/mmrm/main/reference/h_mmrm_tmb_control.html)
...
h_mmrm_tmb_control(optimizer = stats::nlminb,
