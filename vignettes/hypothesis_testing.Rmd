...
For effect $E_2$, it is said to contain $E1$ if
1. they both involve the same continuous covariates
1. $E_2$ have equal or more categorical covariates than $E_1$
...
Please note that this is $2\times 6$ matrix, the rows is the number of coefficients that is associated with `RACE`,
and the columns is the total number of coefficients `r length(coef(fit))`

Then following step 2, we filled in identity matrix in the corresponding submatrix
