...
1. they both involve the same continuous variable (and no more other continuous variable is involvde)
...
$X_2$ stands for columns of $X$ whose ssociated effect contains $F_1$,
$M = I - X_0(X_0^\top X_0)^{-}X_0^\top$,
and $Z^{-}$ stands for the g2 inverse of $Z$.

Note: In `mmrm` package, we do not allow singularity ingeneral, so the g2 inverse is the inverse.
Thuse we can use identity matrix in step 2.

# Type III Hypothese Testing
...
this 1 is devided by the total number of new levels.
...
Please note that in this example we excludes the covariance structure part.
