...
For determining the denominator degrees of freedom (DDF) required for the testing of fixed effects, 
one option is to use the "between-within" method, originally proposed by @schluchter1990small as a small-sample adjustment.

## General definition

Using this method, the DDF are determined by the grouping level at which the term is estimated. Generally, assuming $G$ levels of grouping:
...
that the DDF for the intercept term itself (when it is included) are calculated at the $G+1$ level, i.e. for the intercept we use $DDF_{G+1}$ degrees of freedom.
...
we take the minimum DDF across the involved coefficients as the DDF.
...
* Level 0: The intercept term, assuming the model has been fitted with one. 
  - We use $DDF_2$ degrees of freedom as defined below.
* Level 1: Effects that change between subjects, but not across observations within subjects.
  - These are the "between parameters".
  - The corresponding degrees of freedom are $DDF_1 = N_1 - (N_0 + p_1)$.
  - In words this can be read as:\
    "Between" DDF = "number of subjects" - ("1 if intercept otherwise 0" + "number of between parameters").
* Level 2: Effects that change within subjects.
  - These are the "within parameters".
  - The corresponding degrees of freedom are $DDF_2 = N_2 - (N_1 + p_2)$.
  - In words this can be read as:\
...
And we obtain therefore the degrees of freedom $DDF_g$ at the levels $g=1,2$:

* $DDF_1 = N_1 - (N_0 + p_1) = 197 - (1 + 4) = 192$
* $DDF_2 = N_2 - (N_1 + p_2) = 537 - (197 + 6) = 334$
...
* In SAS, the final effect you list in the `CONTRAST`/`ESTIMATE` statement is used to define the DDF for general contrasts.
