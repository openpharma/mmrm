...
## Satterthwaite degrees of freedom for sandwich covariance

In @bell2002bias the degrees of freedom of sandwich covariance are described.
...
\[
  v = s c(X^\topX)^{-1}\sum_{i}{X_i^\top A_i \epsilon_i \epsilon_i^\top A_i X_i} (X^\topX)^{-1} c^\top
\]

where $s$ takes the value of $\frac{n}{n-1}$, $1$ or $\frac{n-1}{n}$, and $A_i$ takes $I_i$, $(I_i - H_{ii})^{-\frac{1}{2}}$, or $(I_i - H_{ii})^{-1}$
respectively, $c$ is a vector, then $v$ can be decomposed into the a weighted sum of independent $\Chi_1^2$ distribution, where the weights are
the eigen values of $n\times n$ matrix $G$, its element can be represented as
\[
  G_{ij} = g_i V g_j
\]
...
where $\lambda_i$ is the eigen value of matrix $G$.
@bell2002bias also suggests that $V$ can be chosen as identify matrix, so $G_{ij} = g_i  g_j$.
...
\[
  GG_i = (I - H)_i^\top A_i X_i (X^\top X)^{-1}
\]
is a $\sum_i{m_i} \times p$ matrix. With different contrasts, we need only calculate the following
\[
  g_i = GG_i c
\]
