...
than other R implementations regardless of the specified covariance structure.
...
unstructured covariance matrices, are applied to the FEV data 100 times. The
...
fastest procedure, however.


## Marginal Treatment Effect Estimates Comparison

We next estimate the marginal mean treatment effects for each visit number using
the considered mixed model for repeated measures implementations. All `R`
implementations' estimates are reported relative to `PROC MIXED`'s estimates.
...
All `R` procedures produces estimates that are very similar to those output by
`PROC MIXED`, though `mmrm` and `gls` generate the estimates that are closest to
`PROC MIXED`.
