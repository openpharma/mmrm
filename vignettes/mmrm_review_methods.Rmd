...
In this vignette we will show comparisons between `mmrm::mmrm`, `SAS PROC
MIXED`, `nlme::gls`, `lme4::lmer`, and `glmmTMB::glmmTMB` implementations and
capabilities. A primary difference lies in what is supported in terms of
covariance structures. In particular, `SAS PROC MIXED` and `mmrm` are the only
implementations which provide support for many of the most common covariance
structures. `gls`, `lmer` and `glmmtmb` are more limited. `mmmrm` generally
converges more quickly than other procedures regardless of the specified
...
mb <- microbenchmark::microbenchmark(times = 10,
  gls = nlme::gls(FEV1 ~ RACE + ARMCD * AVISIT, data = fev_data, 
    na.action = na.omit,
  ...
  glmmtmb = glmmTMB::glmmTMB(
    ...
    data = fev_data, dispformula = ~ 0, REML = TRUE)
 )
mb %>% summary() %>% dplyr::arrange(median) %>% 
  dplyr::select(expression = expr, median, 
    lower = lq, upper = uq, evaluations = neval) %>%
