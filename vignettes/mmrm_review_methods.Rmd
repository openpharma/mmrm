...
# compute marginal estimates for gls
gls_fit <- gls(
  BCVA_CHG ~ BCVA_BL + RACE + ARMCD * AVISIT, data = bcva_data[complete.cases(bcva_data), ],
  correlation = nlme::corSymm(form = ~ VISITN | USUBJID),
  weights = nlme::varIdent(form = ~ 1|VISITN)
)
gls_ests <- as.data.frame(
...
"
  ...
  proc_mixed_ests <- c(0.539107, 0.724779, 1.011511, 1.104181, 1.383358,
...
## construct the table
bind_cols(
  tibble(Missingness = c("none", "mild", "moderate", "heavy")),
...
```{r convergence-rate-missingness}
    ...
    function(idx) {
      ...
      if (file.exists("sascfg_personal.py")) {
        ...
        results <- tibble(
          ...
          converged = c(mmrm_converged, lmer_converged, gls_converged,
