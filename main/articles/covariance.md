...
\end{pmatrix}
```
with entries $`\sigma_i > 0`$, and the symmetric correlation matrix
$`P`$ is
``` math
P = \begin{pmatrix}
...
$`\sigma = \sigma_1 = \dotsb = \sigma_m`$ gives a “homogeneous”
covariance structure instead.
...
We use different transformations for the compound symmetry and spatial
exponential covariance structures, as explained below.
...
```
and we have in total $`2m - 1`$ variance parameters. Assuming a constant
variance yields a homogeneous ante-dependence covariance structure with
total $`m`$ variance parameters.
...
\end{pmatrix}
```
This means that the correlation between two time points only depends on
...
``` math
\theta = f(\rho) = \mathop{\mathrm{logit}}((\rho + a) / (1 + a))
```
with the inverse
``` math
\rho = f^{-1}(\theta) = \mathop{\mathrm{logit}}^{-1}(\theta) (1 + a) - a.
```
