...
Using "satterthwaite df" the F statistic is close, the d.f. is not.
...
Note: the doolittle method is used to create type 1 contrasts. Not relavent here.
...
So for `AVISIT * ARMCD` the effect should be not affected by the order of arguments.
...
PROC MIXED DATA = fev cl method=reml;
  CLASS AVISIT(ref = 'VIS4') ARMCD(ref = 'PBO') USUBJID;
  MODEL FEV1 = FEV1_BL ARMCD AVISIT FEV1_BL * AVISIT / ddfm=satterthwaite htype=1,2,3;
  REPEATED AVISIT / subject=USUBJID type=ar(1);
RUN;
"
