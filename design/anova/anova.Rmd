...
| AVISIT*ARMCD | 3 | 335.929182055519 | 0.345902681482967 | 0.792143200158601    |
...
get_l_matrix_type2 <- function(fit, var) {
  ...
  all_levels <- vapply(all_vars, function(x) {
    if (is.factor(fit$tmb_data$data[[x]])) length(attr(fit$tmb_data$data[[x]], "levels")) else 2 # only for simplest case of numeric values
  }, FUN.VALUE = 1)
...
get_l_matrix_type3 <- function(fit, var) {
  ...
  all_vars <- row.names(fcts)[which(fcts[, idx] == 1)]
  all_levels <- vapply(all_vars, function(x) {
    length(attr(fit$tmb_data$data[[x]], "levels"))
  }, FUN.VALUE = 1)
  total_levels <- prod(all_levels)
  lvls <- prod(all_levels - 1)
  l_mx <- matrix(0, nrow = lvls, ncol = ncol(mx))
  ...
  for (i in seq_len(ncol(fcts))) {
    if (ods[i] > ods[idx] && all(fcts[, i] >= fcts[, idx])) {
      ...
      a_levels <- vapply(a_vars, function(x) {
        length(attr(fit$tmb_data$data[[x]], "levels"))
      }, FUN.VALUE = 1)
      t_levels <- prod(a_levels)
      l_mx[, which(asg == i)] <- l_mx[, which(asg == idx)] / t_levels * total_levels
    }
  }
  l_mx
}
...
l_mt <- get_l_matrix_type3(fit, "AVISIT")
df_md(fit, l_mt)
```
...
get_l_matrix_type2(fit, "FEV1_BL")
```
