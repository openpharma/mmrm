...
\[
  \frac{\partial{\Sigma\Sigma^{-1}}}{\partial{\theta_g}}\\
  = \frac{\partial{\Sigma}}{\partial{\theta_g}}\Sigma^{-1} + \Sigma\frac{\partial{\Sigma^{-1}}}{\partial{\theta_g}} \\
  = 0
\]
\[
  \frac{\partial{\Sigma^{-1}}}{\partial{\theta_g}} = - \Sigma^{-1} \frac{\partial{\Sigma}}{\partial{\theta_g}}\Sigma^{-1}
\]
...
\[
  \frac{\partial{\Sigma_i}}{\partial{\theta_g}} = S_i^\top \frac{\partial{\Sigma}}{\partial{\theta_g}} S_i
\]
...
The derivative of the $\Sigma_i^{-1}$, $\frac{\partial\Sigma_i^{-1}}{\partial{\theta_g}}$ can be calculated through $\Sigma_i^{-1}$ and $\frac{\partial{\Sigma_i}}{\partial{\theta_g}}$
using the above.
...
When we have only one coefficient to test, then $C$ is a column vector containing a single $1$ inside.
The hypothesis testing is still the same and falls back to Satterthwaite method.
