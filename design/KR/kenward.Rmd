...
## Mathmatical Details of Kenward-Roger method

The mathmatical derivation of the Kenward-Roger method is based on the Taylor expansion of the obtained covariance matrix
of $\hat\beta$ to get a more accurate estimate. All these derivation are based on the restricted maximum likelihood.
Following the same [notation](algorithms.html#covariance-matrix-model), the covariance matrix, $\Sigma$ can be represented as
a function of covariance matrix parameters, $\Sigma(\theta)$
Here after model fitting with `mmrm`, we obtain the estimate $\hat\beta = \Phi(\hat\theta)X^\top\Sigma(\hat\theta)^{-1}Y$,
where $\Phi(\theta) = \left\{X^\top \Sigma(\theta)^{-1} X\right\} ^{-1}$.
However, Kackar and Harville(@kackar1984) suggests that although the $\hat\beta$ is unbiased for $\beta$, the covariance matrix,
$V[\hat\beta] = \left\{X^\top \hat\Sigma X\right\}^{-1}$ can be biased. They showed that the variability of $\hat\beta$ can be partitioned into
two components,
...
Based on a Talor series expansion around $\theta$, $\Lambda$ can be approximated by
...
where
\[
  P_j = X^\top \frac{\partial{\Sigma^{-1}}}{\partial \theta_j} X
\]
\[
  Q_{jk} = X^\top \frac{\partial{\Sigma^{-1}}}{\partial \theta_j} \Sigma \frac{\partial{\Sigma^{-1}}}{\partial \theta_k} X
\]

Again, based on a Talor series expansion about $\theta$, Kenward and Roger(@kenward1997) shows that
\[
...
where
\[
  R_{jk} = X^\top\Sigma^{-1}\frac{\partial^2\Sigma}{\partial{\theta_j}\partial{\theta_k}} \Sigma^{-1} X
\]
...
where $\Sigma(\hat\theta)$ replaces $\Sigma(\theta)$ in the righthand side, and $W$ is the inverse of the Hessian matrix of the objective function.

### Special Considerations for mmrm models

In mmrm models, $\Sigma$ is a block-diagonal matrix, hence we can calculate  $P$, $Q$ and $R$ for each subject and add them up.
...
### Derivative of the sigma

The derivative of the sigma can be calculated through the derivatives of Cholesky factor(obtained through automatic differentiation)
...
### Derivative of the sigma inverse

The derivative of simga inverse can be calculated through
...
\[
P_j = \begin{pmatrix}
P_{j, 1} & 0 & \dots & \dots & 0 \\
0 & P_{j, 2} & 0 & \dots & 0\\
\vdots & & \ddots & & \vdots \\
\vdots & & & \ddots & \vdots \\
0 & \dots & \dots & 0 & P_{j, B}
\end{pmatrix}
\]

Use $P_{j, b}$ to denote the block diagonal part for group $b$, we have
\[
  P_{j,b} = \sum_{g(i) = b}{P_{ij}} = \sum_{i=1}^{N}{X_i^\top \frac{\partial{\Sigma_i^{-1}}}{\partial \theta_j} X_i}
\]
...
there are no mathmatical differences as they are constant, and having $G_i$ in addition to $S_i$
does not change the algorithms and we can simply do it.

## Inference

Suppose we are testing the $c$ linear combination of $\beta$, $C\beta$, we can use the following Wald-type statistic

\[
  F = \frac{1}{l} (\hat\beta - \beta)^\top  C (C^\top \hat\Phi_A C)^{-1} C^\top (\hat\beta - \beta)
\]
and
\[
  F^* = \lambda F
