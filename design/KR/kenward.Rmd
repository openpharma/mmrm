...
\[
  \Lambda \simeq \Phi \left\{\sum_{g=1}^k{\sum_{h=1}^k{W_{gh}(Q_{gh} - P_g \Phi P_h)} }\right\} \Phi
\]
where
\[
  P_g = X^\top \frac{\partial{\Omega^{-1}}}{\partial \theta_g} X
\]
\[
  Q_{gh} = X^\top \frac{\partial{\Omega^{-1}}}{\partial \theta_g} \Omega \frac{\partial{\Omega^{-1}}}{\partial \theta_h} X
\]

$W$ is the inverse of the Hessian matrix of the objective function in fitting the mmrm model.

Again, based on a Taylor series expansion about $\theta$, Kenward and Roger(@kenward1997) show that
\[
  \hat\Phi \simeq \Phi + \sum_{g=1}^k{(\hat\theta_g - \theta_g)\frac{\partial{\Phi}}{\partial{\theta_g}}} + \frac{1}{2} \sum_{g=1}^k{\sum_{h=1}^k{(\hat\theta_g - \theta_g)(\hat\theta_h - \theta_h)\frac{\partial^2{\Phi}}{\partial{\theta_g}\partial{\theta_h}}}}
\]
Ignoring the possible bias in $\hat\theta$,
\[
  E(\hat\Phi) \simeq \Phi + \frac{1}{2} \sum_{g=1}^k{\sum_{h=1}^k{W_{gh}\frac{\partial^2{\Phi}}{\partial{\theta_g}\partial{\theta_h}}}}
\]
Using previously defined notations, this can be further written as
\[
  \frac{\partial^2{\Phi}}{\partial{\theta_g}\partial{\theta_h}} = \Phi (P_g \Phi P_h + P_h \Phi P_g - Q_{gh} - Q_{hg} + R_{gh}) \Phi
\]
where
\[
  R_{gh} = X^\top\Omega^{-1}\frac{\partial^2\Omega}{\partial{\theta_g}\partial{\theta_h}} \Omega^{-1} X
\]

substituting $\Phi$ and $\Lambda$ back to the $\hat\Phi_A$, we have

\[
  \hat\Phi_A = \hat\Phi + 2\hat\Phi \left\{\sum_{g=1}^k{\sum_{h=1}^k{W_{gh}(Q_{gh} - P_g \hat\Phi P_h - \frac{1}{4}R_{gh})} }\right\} \hat\Phi
\]

where $\Omega(\hat\theta)$ replaces $\Omega(\theta)$ in the righthand side.
...
\[
  P_g = \sum_{i=1}^{N}{P_{ig}} = \sum_{i=1}^{N}{X_i^\top \frac{\partial{\Sigma_i^{-1}}}{\partial \theta_g} X_i}
\]

\[
  Q_{gh} = \sum_{i=1}^{N}{Q_{igh}} = \sum_{i=1}^{N}{X_i^\top \frac{\partial{\Sigma_i^{-1}}}{\partial \theta_g} \Sigma_i \frac{\partial{\Sigma_i^{-1}}}{\partial \theta_h} X_i}
\]

\[
  R_{gh} = \sum_{i=1}^{N}{R_{igh}} = \sum_{i=1}^{N}{X_i^\top\Sigma_i^{-1}\frac{\partial^2\Sigma_i}{\partial{\theta_g}\partial{\theta_h}} \Sigma_i^{-1} X_i}
\]
...
\[
  \frac{\partial{\Sigma}}{\partial{\theta_g}} = \frac{\partial{LL^\top}}{\partial{\theta_g}} = \frac{\partial{L}}{\partial{\theta_g}}L^\top + L\frac{\partial{L^\top}}{\partial{\theta_g}}
\]

\[
  \frac{\partial^2{\Sigma}}{\partial{\theta_g}\partial{\theta_h}} = \frac{\partial^2{L}}{\partial{\theta_g}\partial{\theta_h}}L^\top + L\frac{\partial^2{L^\top}}{\partial{\theta_g}\partial{\theta_h}} + \frac{\partial{L}}{\partial{\theta_g}}\frac{\partial{L^T}}{\partial{\theta_h}} + \frac{\partial{L}}{\partial{\theta_h}}\frac{\partial{L^\top}}{\partial{\theta_g}}
\]
...
\[
  \frac{\partial^2{\Sigma_i}}{\partial{\theta_g}\partial{\theta_h}} = S_i^\top \frac{\partial^2{\Sigma}}{\partial{\theta_g}\partial{\theta_h}} S_i
\]
...
When fitting grouped `mmrm` models, the covariance matrix for subject i of group $g(i)$, can be written as $\Sigma_{g(i)}$.
...
\[
R_g = \begin{pmatrix}
R_{j, 1} & 0 & \dots & \dots & 0 \\
0 & R_{j, 2} & 0 & \dots & 0\\
\vdots & & \ddots & & \vdots \\
\vdots & & & \ddots & \vdots \\
0 & \dots & \dots & 0 & R_{j, B}
\end{pmatrix}
\]

Use $P_{j, b}$ to denote the block diagonal part for group $b$, we have
\[
  P_{g,b} = \sum_{g(i) = b}{P_{ig}} = \sum_{g(i) = b}{X_i^\top \frac{\partial{\Sigma_i^{-1}}}{\partial \theta_g} X_i}
\]

\[
  Q_{gh,b} = \sum_{g(i) = b}{Q_{igh}} = \sum_{g(i) = b}{X_i^\top \frac{\partial{\Sigma_i^{-1}}}{\partial \theta_g} \Sigma_i \frac{\partial{\Sigma_i^{-1}}}{\partial \theta_h} X_i}
\]

\[
  R_{gh,b} = \sum_{g(i) = b}{R_{igh}} = \sum_{g(i) = b}{X_i^\top\Sigma_i^{-1}\frac{\partial^2\Sigma_i}{\partial{\theta_g}\partial{\theta_h}} \Sigma_i^{-1} X_i}
\]
...
\[
  A_1 = \sum_{j=1}^k{\sum_{k=1}^k{W_{gh} tr(M \Phi P_g \Phi) tr(M \Phi P_h \Phi)}}
\]

\[
  A_2 = \sum_{j=1}^k{\sum_{k=1}^k{W_{gh} tr(M \Phi P_g \Phi M \Phi P_h \Phi)}}
\]
