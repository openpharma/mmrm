...
q <- function(x, sigma, inv_sig_deri) {
  lapply(inv_sig_deri, function(s) {
    lapply(inv_sig_deri, function(t) {
      t(x) %*% s %*% sigma %*% t %*% x
    })
  })
}
...
w <- function(inv_sig_deri, sigma, p, q, v) {
  ...
  for (i in seq_len(r)) {
    for (j in seq_len(r)) {
      ret[i, j] <- tr(inv_sig_deri[[i]] %*% sigma %*% inv_sig_deri[[j]] %*% sigma) - tr(2 * v %*% q[[i]][[j]] - v %*% p[[i]] %*% v %*% p[[j]])
    }
  }
  return(solve(ret / 2))
}
...
derivative_ar1 <- function(visits, sig, rho, order = 1) {
  # r: number of parameters
  ...
  if (order == 1) {
    psig <- rho ^ dis
    prho <- rho ^ (dis - 1) * dis * sig
    return(list(sig = psig, rho = prho))
  }
  if (order == 2) {
    sigsig <- matrix(0, ncol = visits, nrow = visits)
    sigrho <- rho ^ (dis - 1) * dis
    rhosig <- rho ^ (dis - 1) * dis
    rhorho <- rho ^ (dis - 2) * dis * (dis - 1) * sig
...
d1 <- derivative_ar1(component(fit, "n_timepoints"), theta1(fit$theta_est[1]), theta2(fit$theta_est[2]), order = 1)
d2 <- derivative_ar1(component(fit, "n_timepoints"), theta1(fit$theta_est[1]), theta2(fit$theta_est[2]), order = 2)
# all subjects x
x <- component(fit, 'x')
...
w_sparse <- w(sigma_inv_d1, sigma_sparse, p_sparse, q_sparse, fit$beta_vcov)
