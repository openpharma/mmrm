...
anova.mmrm <- function (object, ..., test = TRUE, refit = FALSE) {
  ...
  if (length(fits) > 1L) {
    ...
    # If we're refitting...
    if (refit) {
      # ...if the data are the same, only refit REML models
      if (h_check_same_data(fits)) {
        for (i in which(h_reml_fits_lgl(fits)))
          fits[[i]] <- h_refit_mmrm(fits[[i]])
      } else {
        # ...if the data are not all the same, refit all with the common data
    ...
    if (test) {
      ...
      # If we didn't refit, throw errors if not all data are the same or if any
      # REML models are present
      if (!refit) {
        if (!h_check_same_data(fits)) {
          stop(
        ...
        if (any(h_reml_fits_lgl(fits))) {
          stop(
            "Likelihood ratio testing cannot be performed on fits using REML.",
            " At least one fit uses REML and refit = FALSE.",
            " Consider setting test = FALSE or refit = TRUE.",
            call. = FALSE
          )
        }
...
# Returns a logical vector identifying the reml fits
h_reml_fits_lgl <- function(fits) {
  vapply(fits, component, logical(1L), "reml")
}
...
# Refit a single mmrm model. Always forces reml = FALSE. If data is included, it
# forces the data argument of mmrm() to point to the new data set.
h_refit_mmrm <- function(fit, data = NULL) {
  
  # Grab the model call
  expr <- component(fit, "call")
  
  # Force the call not to use reml
  expr[["reml"]] <- FALSE
  ...
  if (!is.null(data)) {
    ...
    # bind the new data to the new name in the model's environment
    env[[data_name]] <- data
...
h_generate_new_name <- function(x, env) {
  ...
  while (exists(x[length(x)], envir = env, inherits = TRUE)) {
    ...
    x <- make.names(c(x, x[1L]), unique = TRUE)
  }
...
h_df_pull_columns_to_front <- function(df, cols) {
  df[union(cols, colnames(df))]
}
...
h_assert_nested_models <- function(model_basic, model_augmented,
                                   model_basic_name = checkmate::vname(model_basic),
                                   model_augmented_name =
                                     checkmate::vname(model_augmented)) {
...
Now, use multiple models. Note that the likelihood ratio test is included by default. We specify `refit = TRUE` because `mmrm()` uses REML by default, which doesn't work with the likelihood ratio test.

```{r}
anova(fit_reduced, fit, fit_full, refit = TRUE)
```

If we don't want to refit, we can run `anova()` but specify `test = FALSE`. This will result in fewer diagnostics.

```{r}
anova(fit_reduced, fit, fit_full, test = FALSE, refit = FALSE)
```

Now, let's create a model fit that's based on more data:
...
Let's also refit the first three models, this time specifying `reml = FALSE`.
...
Now we can obtain LRT results even with `refit = FALSE`.

```{r}
anova(fit_reduced_ml, fit_ml, fit_full_ml, test = TRUE, refit = FALSE)
```

But we can't do this if we slot in the new `fit_more_data` because although it didn't use REML, it still has a different underlying data set:

```{r}
anova(fit_reduced_ml, fit_more_data, fit_full_ml, test = TRUE, refit = FALSE)
```

Setting `refit = TRUE` makes it work:

```{r}
anova(fit_reduced_ml, fit_more_data, fit_full_ml, test = TRUE, refit = TRUE)
```
