...
data <-
  fev_data %>%
  filter(VISITN == min(VISITN)) %>%
  mutate(FEV1 = FEV1_BL, AVISIT = "BASELINE", VISITN = 0L) %>% 
  bind_rows(fev_data, .) %>%
  select(!c(FEV1_BL, VISITN2, WEIGHT)) %>%
  arrange(USUBJID, VISITN) %>%
  as_tibble() %>% 
  mutate(
    VISITX = VISITN * 12,
    TIMEX = VISITX + runif(n = n(), min = -1, max = 1),
    TIMEN = VISITN,
    TIMEF =
      ordered(as.character(TIMEN), levels = as.character(sort(unique(TIMEN))))
  ) %>% 
  select(FEV1, AVISIT, VISITX, VISITN, TIMEX, TIMEN, TIMEF, USUBJID, ARMCD,
         RACE, SEX)
basis <- splines::ns(data$TIMEX, df = 2, Boundary.knots = c(0, max(data$TIMEX)))
spline <- function(TIMEX, column) {
  as.numeric(predict(basis, TIMEX)[, column])
}
```
...
logLik.mmrm_tmb <- function(object, ...) {
  ...
  nall <- component(object, "n_obs")

  # was REML used (ML was used if FALSE)
  reml_used <- component(object, "reml")

  # Number of estimated coefficients (excluding aliased coefficients)
  n_fixed_effects <- length(component(object, "beta_est"))

  # Adjusted number of observations when REML is used
  nobs <- nall - reml_used * n_fixed_effects

  # Number of parameters
  df <- component(object, "n_theta")

  structure(out, nall = nall, nobs = nobs, df = df, class = "logLik")
}
```
...
| `refit` | Logical indicating whether the models should be refitted before comparing. This is ignored if the models all have the same underlying data set. If `refit = FALSE` and the models have different underlying data sets, an error will be thrown. Defaults to `FALSE`. |
...
If the user supplied `test = TRUE`, the model fits are subject to further tests to ensure they are nestable. If the data sets for the fits are not the same and the user supplied `refit = TRUE`, the data sets are harmonized and the models are refitted using the harmonized data set. The pairs of models then undergo likelihood ratio testing, and the results are augmented into the data frame of results.
...
anova.mmrm <- function (object, ..., test = TRUE, refit = FALSE) {
  ...
  if (...length()) {
    ...
    if (refit) {
      fits <- h_fits_data_identical(fits, refit = TRUE)
    }
    ...
    out <-
      data.frame(
        call = vapply(fit_calls, deparse1, character(1L)),
    ...
    if (test) {
      ...
      if (!refit && !h_fits_data_identical(fits, refit = FALSE)) {
        stop("Not all data sets underlying the model fits are identical and ",
             "refit = FALSE.", call. = FALSE)
      }
      
      # Label for the pair being compared (e.g., "3 vs 4")
      out$test <- c(NA, paste(head(out$Model, -1), "vs", tail(out$Model, -1)))
      
      # Calculate the likelihood ratio between the pairs using base::diff()
      out$likelihood_ratio <- c(NA, diff(out$logLik))
      
      # Calculate the p-value using the ratios and differences in df
      out$p_value <-
        pchisq(
          2 * abs(out$likelihood_ratio),
          df = abs(diff(out$df)), 
          lower.tail = FALSE
        )
    }
...
```{r}
# Compares the data sets underlying a list of fits. 

# If refit = FALSE, this is a predicate function indicating whether all the data
# sets are the same.

# If refit = TRUE, all data sets are harmonized, the models are refit using the
# harmonized data set, and the refits are returned.
h_fits_data_identical <- function(fits, refit = FALSE) {
  
  # Grab the data frames behind the model fits into a separate list
  dfs <- lapply(fits, component, "full_frame")
  
  # Sort the first data frame. Also store it in data
  data <- dfs[[1L]] <- h_df_sort_all(dfs[[1L]])
  
  # Begin by assuming all fits' data are the same
  all_data_same <- TRUE
  
  # Iterate through remaining fits
  for (i in seq_along(fits)[-1L]) {
    
    # Sort the current fit's data set, putting the previous fit's columns first
    dfs[[i]] <-
      h_df_sort_all(
        dfs[[i]][
          union(colnames(dfs[[i - 1]]), colnames(dfs[[i]]))
        ]
      )
    
    # If the shared columns in the current and previous data sets are not
    # identical...
    if (
      !identical(
        h_df_sort_all(dfs[[i - 1]]),
        h_df_sort_all(dfs[[i]][colnames(dfs[[i - 1]])])
      )
    ) {
      
      # If we're refitting...
      if (refit) {
        # ...mark that not all data sets are the same...
        all_data_same <- FALSE
        # ...and merge the data sets, storing the merged data set in df[[i]] and
        # in data
        data <- dfs[[i]] <-
          h_df_sort_all(merge(dfs[[i]], dfs[[i - 1]], all = TRUE, sort = FALSE))
      } else {
        # If we're not refitting, just return FALSE
        return(FALSE)
      }
  ...
  if (refit) {
    ...
    if (all_data_same) {
      # If all data were the same, return the original model fits
      out <- fits
    } else {
      # Otherwise, refit the models and return
      out <- Map(h_refit_mmrm_with_data, fits, dfs, MoreArgs = list(data))
    }
    
  } else {
    # If we aren't refitting and we've gotten this far, return TRUE
    out <- TRUE
  }
...
# Refit a single model using a new data set. Does not refit if original data is
# the same as the new data.
h_refit_mmrm_with_data <- function(fit, current_df, new_df) {
  # If the model fit's current data set is not the same as the new one...
  if (!identical(current_df, new_df)) {
    # Grab the model call and replace its "data" argument with the new data
    expr <- component(fit, "call")
    expr[["data"]] <- new_df
    # Grab the environment of the model's formula
    envir <- environment(fit[["formula_parts"]][["full_formula"]])
    # Evaluate the model call in the environment of the original fit's formula
    fit <- eval(call, envir)
  }
...
h_assert_nested_models <- function(model_basic,
                                   model_augmented,
                                   model_basic_name = vname(model_basic),
                                   model_augmented_name =
                                     vname(model_augmented)) {
  ...
  if (!identical(model_basic[["visit_var"]], model_augmented[["visit_var"]])) {
    stop(model_basic_name, " and ", model_augmented_name,
         " must have the same visit variable.")
  }

  if (!identical(model_basic[["subject_var"]], model_augmented[["subject_var"]])) {
    stop(model_basic_name, " and ", model_augmented_name,
         " must have the same subject variable.")
  }

  if (!identical(model_basic[["group_var"]], model_augmented[["group_var"]])) {
    stop(model_basic_name, " and ", model_augmented_name,
         " must have the same grouping variable.")
  }
  ...
  switch(
    ...
    "identical" = 
      if (
        h_check_cov_struct_nesting(model_basic, model_augmented) == "identical"
      ) {
        warning("Models have identical covariates and covariance structures.")
      },
    "nested" = 
      if (
        h_check_cov_struct_nesting(model_basic, model_augmented) == "nested"
      ) {
        stop("Models cannot have both nested covariates and nested covariance structures.",
             call. = FALSE)
      }
  )
}
...
h_check_covar_nesting <- function(model_basic,
                                  model_augmented,
                                  basic_name = checkmate::vname(model_basic),
                                  aug_name = checkmate::vname(model_augmented)) {
  ...
  if (!all(basic_covars %in% aug_covars)) {
    stop("The covariates in ", basic_name,
         " must be a subset of the covariates in ", aug_name, " .")
  }

  if (all(aug_covars %in% basic_covars)) {
    "identical"
  } else {
    "nested"
  }
...
# identical).
h_check_cov_struct_nesting <- function(model_basic,
                                       model_augmented,
                                       basic_name = vname(model_basic),
                                       aug_name = vname(model_augmented)) {
  ...
  if (identical(basic_cov_struct, aug_cov_struct)) {
    ...
    stop(
      "The covariance structure of ", aug_name,
      " must be identical to or a special case of the covariance structure of ",
      basic_name, ". The covariance structure ", aug_cov_struct,
      " is not a special case of ", basic_cov_struct, ".",
      call. = FALSE
    )
  }
}
...
# Sorts a data frame's columns alphabetically, then sorts its rows by each
# column in order
h_df_sort_all <- function(df) {
  cols <- sort(colnames(df))
  Reduce(function(df, col) df[order(df[[col]]), ], rev(cols), init = df[cols])
}
```

# Examples

Using a modified version of the `fit_reduced` and `fit_full` models in the tutorial:

```{r}
fit_reduced <- 
  mmrm(
    FEV1 ~
      I(spline(TIMEX, 1))*SEX + I(spline(TIMEX, 2))*SEX +
      RACE + SEX +
      us(TIMEF | USUBJID),
    data = data
  )
...
fit_full <- 
  mmrm(
    FEV1 ~ 
      I(spline(TIMEX, 1))*SEX + I(spline(TIMEX, 2))*SEX +
      I(spline(TIMEX, 1)):ARMCD + I(spline(TIMEX, 2)):ARMCD +
      RACE + SEX +
      us(TIMEF | USUBJID),
    data = data
  )
...
Now, use two models. Note that the likelihood ratio test is included by default:
...
Tautologically, all covariance structures are special cases of an unstructured covariance, and a model with a covariance structure can be considered "nested" within an otherwise equivalent model without a covariance structure.
...
We will now restrict the correlations in one covariance structure can produce another covariance structure in this set, demonstrating that they are nested. We will only do this for the homogeneous structures, since the heterogeneous ones plainly follow the same pattern.
