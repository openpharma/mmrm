...
h_contrast_var_fun <- function(model, L) {
  ...
  function(theta) {
    ...
    h_quadratic_form(L, covbeta)
  }
}

contrast_var_fun <- h_contrast_var_fun(result$model, L)
sqrt(contrast_var_fun(theta_est))
```
...
```{r}
num_grad_contrast_var <- numDeriv::grad(contrast_var_fun, theta_est)
```
...
```{r}
num_v_denom <- h_quadratic_form(num_grad_contrast_var, result$vcov_theta)
num_v_denom
```
