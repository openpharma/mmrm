...
fixed effects when plugging in the estimated variance parameters, and we
get a different result when using other variance parameters.

```{r}
covbeta_fun <- h_covbeta_fun(mod_refit)
theta_est <- getME(mod_refit, "theta")
all.equal(
  covbeta_fun(theta = theta_est),
  unname(vcov(mod_refit)$cond)
)
range(covbeta_fun(theta = theta_est) - covbeta_fun(theta = theta_est * 0.9))
```
...
```{r}
jac_example <- h_general_jac_list(covbeta_fun, theta_est)
```
...
```{r}
h_quadratic_form <- function(vec, mat) {
  assert_numeric(vec, any.missing = FALSE)
  assert_matrix(mat, mode = "numeric", any.missing = FALSE, nrows = length(vec), ncols = length(vec))
  sum(vec * (mat %*% vec))
}

h_quadratic_form(1:2, matrix(1:4, 2, 2))
```
...
df_1d <- function(object, L) {
  ...
  contrast_var <- h_quadratic_form(L, object$vcov_beta)
  contrast_grad <- h_gradient(object$jac_list, L)

  v_numerator <- 2 * contrast_var ^ 2
  v_denominator <- h_quadratic_form(contrast_grad, object$vcov_theta)

  list(
    est = contrast_est,
    se = sqrt(contrast_var),
    df = v_numerator / v_denominator
  )
}
```
...
So unfortunately that does not give the same result.
