...
# h_mmrm_tmb_assert_fit ----

test_that("h_mmrm_tmb_assert_fit passes as expected for sane fit result", {
  tmb_object <- list(
    fn = function(par) sum(par),
  ...
  result <- expect_silent(h_mmrm_tmb_assert_fit(tmb_object, tmb_fit))
  expect_null(result, expected)
})

test_that("h_mmrm_tmb_assert_fit raises eigenvalue error as expected", {
  tmb_object <- list(
    fn = function(par) sum(par),
    gr = function(par) par * 2,
    he = function(par) "bla",
...
test_that("h_mmrm_tmb_assert_fit warns for negative eigenvalues as expected", {
  tmb_object <- list(
    fn = function(par) sum(par),
    gr = function(par) par * 2,
...
test_that("h_mmrm_tmb_assert_fit warns if convergence code signals non-convergence", {
  tmb_object <- list(
    fn = function(par) sum(par),
    gr = function(par) par * 2,
    he = function(par) tcrossprod(par) + diag(1, 5, 5),
...
test_that("mmrm_tmb works as expected in a simple model without covariates and ML", {
  ...
  expect_named(
    result$object,
    c("par", "fn", "gr", "he", "hessian", "method", "retape", "env", "report", "simulate")
  )
  expect_named(
    result$fit,
  ...
  expect_equal(2 * result$fit$objective, 3700.68707570)
  result_reports <- result$object$report()
  expect_equal(sqrt(1 / as.numeric(result_reports$XtWX)), 0.3499, tolerance = 1e-4)
  expect_equal(result_reports$beta, 42.8367, tolerance = 1e-4)
  result_cov_tri <- result_reports$cov_report[lower.tri(result_reports$cov_report)]
...
test_that("mmrm_tmb works as expected in a simple model without covariates and REML", {
  ...
  expect_equal(2 * result$fit$objective, 3700.94648570)
  result_reports <- result$object$report()
  expect_equal(sqrt(1 / as.numeric(result_reports$XtWX)), 0.3509, tolerance = 1e-4)
  expect_equal(result_reports$beta, 42.8338, tolerance = 1e-4)
  result_cov_tri <- result_reports$cov_report[lower.tri(result_reports$cov_report)]
