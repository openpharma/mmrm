...
test_that("component works as expected for mmrm_tmb objects", {
  ...
  expect_identical(
    component(object_mmrm_tmb, "varcor"),
    component(object_mmrm_tmb)$varcor
  )
  expect_identical(
    component(object_mmrm_tmb, "beta_vcov"),
    object_mmrm_tmb$beta_vcov
  )
...
test_that("component returns adjusted beta vcov matrix for Kenward-Roger adjusted mmrm", {
  ...
  expect_identical(
    component(object_mmrm_kr, "beta_vcov"),
    object_mmrm_kr$beta_vcov_adj
  )
})
...
test_that("component returns empirical beta vcov matrix", {
  ...
  expect_identical(
    component(object_mmrm_emp, "beta_vcov"),
    object_mmrm_emp$beta_vcov_adj
  )
})
...
test_that("component returns Jackknife beta vcov matrix", {
  ...
  expect_identical(
    component(object_mmrm_jack, "beta_vcov"),
    object_mmrm_jack$beta_vcov_adj
  )
})
...
test_that("component returns coefficient vectors as expected in rank deficient model", {
  ...
  expect_named(
    result,
    c("(Intercept)", "RACEBlack or African American", "RACEWhite", "ones")
  )
...
test_that("component can return xlev also when there is a combined response in the model", {
  object <- mmrm(
