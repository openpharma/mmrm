...
Type objective_function<Type>::operator() ()
{
  ...
  DATA_SPARSE_MATRIX(distance);    // Coordinates matrix (block diagonal sparse matrix).
  ...
  for (int i = 0; i < n_subjects; i++) {
    ...
    matrix<Type> cov_i = get_spatial_covariance_lower_chol(theta_i, distance_i);
  ...
  REPORT(beta_vcov);
  matrix<Type> standard_dist(2, 2);
  matrix<Type> covariance_lower_chol(2 * n_groups, 2);
  standard_dist << 0, 1, 1, 0;
  for (i = 0; i < n_groups; i ++) {
    matrix<Type> covariance_lower_chol_group = get_spatial_covariance_lower_chol(theta.segment(2 * i, theta_size), standard_dist);
    covariance_lower_chol << covariance_lower_chol.block(0, 0, 2 * i, theta.size), covariance_lower_chol_group
  }
