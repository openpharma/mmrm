...
context("cho_jacobian") {
  test_that("cho_jacobian works as expected") {
    ...
    expect_equal_vector<vector<double>>(result, expected);
  }
...
context("derivatives_nonspatial struct works as expected") {
  test_that("derivatives_nonspatial struct correct sigma, inverse and derivatives") {
    ...
    expect_equal_matrix<matrix<double>>(matrix<double>(selmat), matrix<double>(mychol.get_sel_mat(v1)));
    expect_equal_matrix<matrix<double>>(matrix<double>(selmat * full_sigma.block(0,0,4,4) * selmat.transpose()), matrix<double>(part_sigma.block(0,0,3,3)));
    ...
    expect_equal_matrix<matrix<double>>(expected_inverse, full_inverse);
    ...
    expect_equal_matrix<matrix<double>>(expected_v1_inverse, v1_inverse);
    ...
    expect_equal_matrix<matrix<double>>(matrix<double>(derivative1.block(0, 0, 3, 3)), expected_derivative1);
    ...
    expect_equal_matrix<matrix<double>>(matrix<double>(derivative2.block(0, 0, 3, 3)), expected_derivative2);
    auto inverse_derivative = mychol.get_inverse_derivative(v1, dist);
    expect_equal_matrix<matrix<double>>(matrix<double>(inverse_derivative.block(0, 0, 3, 3)), matrix<double>(- v1_inverse * derivative1.block(0, 0, 3, 3) * v1_inverse));
  }
}
...
context("derivatives_sp_exp struct works as expected") {
  test_that("derivatives_sp_exp struct gives correct sigma, inverse and derivatives") {
    ...
    expect_equal_matrix<matrix<double>>(sigma, expected_sigma);
    ...
    expect_equal_matrix<matrix<double>>(sigma_d1, expected_sigma_d1);
    ...
    expect_equal_matrix<matrix<double>>(sigma_d2, expected_sigma_d2);
    ...
    expected_sigma_inv << 
      ...
      0, -1.169564,  1.367879;
    expect_equal_matrix<matrix<double>>(sigma_inv, expected_sigma_inv);
  }
}