test_that("coverage_fun computes DGP-specific coverage", {
  ...
  coverage_tbl <- coverage_fun(fit_results, true_params)

  # make sure that all required columns are present
  expect_equal(
    colnames(coverage_tbl),
    c(".dgp_name", ".method_name", "n_obs", "coefficient", "coverage")
  )

  # make sure that all of the coefficients represented
  expect_equal(
    unique(coverage_tbl$coefficient),
    paste0("trt_visit_num", str_pad(seq_len(10), width = 2, pad = "0"))
  )

  # ensure that the bias column is numeric and between zero and one
  expect_equal(is.numeric(coverage_tbl$coverage), TRUE)
  expect_equal(all(coverage_tbl$coverage %in% c(0, 1)), TRUE)
