test_that("type_2_error_rate_fun computes DGP-specific type 2 error rates", {
  ...
  type_2_error_rate_tbl <- type_2_error_rate_fun(fit_results, true_params)

  # make sure that all required columns are present
  expect_equal(
    colnames(type_2_error_rate_tbl),
    c(".dgp_name", ".method_name", "n_obs", "coefficient", "type_2_error_rate")
  )

  # only DGPs with non-zero effects are present
  expect_equal(all(type_2_error_rate_tbl$.dgp_name == "eff"), TRUE)

  # make sure that all of the coefficients represented
  expect_equal(
    unique(type_2_error_rate_tbl$coefficient),
    paste0("trt_visit_num", str_pad(seq_len(10), width = 2, pad = "0"))
  )

  # ensure that the type_1_error_rate column is numeric
  expect_equal(is.numeric(type_2_error_rate_tbl$type_2_error_rate), TRUE)
