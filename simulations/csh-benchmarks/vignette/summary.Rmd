...
to a difficult-to-diagnose re-labelling issue performed by SAS "under the hood".
...
`2`, ..., `10`. These factor levels are not set explicitly. The second dataset
is identical to the first, save that `time`'s factor labels are explicitly
provided. The third dataset's `time` variable is a factor with levels `01`,
`02`, ..., `10`. These factors are not set explicitly. Finally, a fourth dataset
is considered in which `time` is a numeric variable.
...
set.seed(123)
rct_data_not_padded_explicit_lvls <- rct_dgp_fun(explicit_time_levels = TRUE)
rct_data_not_padded_explicit_lvls$time %>% head(10)
set.seed(123)
...
proc_mixed_fun(
  participant = rct_data_not_padded_explicit_lvls$participant,
  time = rct_data_not_padded_explicit_lvls$time,
  y = rct_data_not_padded_explicit_lvls$y,
  trt = rct_data_not_padded_explicit_lvls$trt
)
...
with padded `time` aren't padded. Alternatively, time points might be fed to SAS
as numerics to avoid this reordering issue. This behaviour should be taken into
account when comparing `mmrm()` and `PROC MIXED` ouputs.
